...
initial: [
	...
	default:
		(
			flags/set 'stripe? true
			feed emit [<section> <div class="container">]
		)
		continue preamble
...
normal: inherit preamble [
	sect1:
		(
			feed
			emit either flags/set 'stripe? not flags/get 'stripe? [
				<section class="stripe">
			][
				<section class="stripe dark">
			]
...
in-quote: inherit in-block [
	quote-out: (if data [emit <h4> emit-inline data emit </h4>]) return
]
